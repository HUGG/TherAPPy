...
try:
    ...
    print('f2py -c AFT_model_core.f90 -m AFT_model_core')
    print('-' * 20)
...
def simulate_AFT_annealing(timesteps_input, temperature_input, kinetic_value,
    ...
    if np.isnan(rmr0) is True or rmr0 <= rmr0_min:
        ...
        print('!! warning, rmr0 value exceeds most resistant apatite in the Carlson (1999) dataset')
    ...
    if use_fortran_algorithm is True:
        ...
        try:
            ...
            if verbose is True:
                print('using python reduced track length function instead of fortran')
